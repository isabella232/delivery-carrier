...
class PostlogisticsWebService(object):
    ...
    def _prepare_data(self, lang, frankingLicense, post_customer,
        ...
        # Without following lines label generation is broken.
        # TODO: Must be fixed correctly
        # Seems we have invalid old values for przl, how it works?
        data['item']['attributes']['przl'] = ['PRI']
        # weight is a new required field
        data['item']['attributes']['weight'] = 5000
        # Seems zip can only take 4 or 6 numbers
        # In France we have 5 numbers, then it failed
        # Is it normal?
        data['item']['recipient']['zip'] = "1030"
        data['item']['recipient']['country'] = "CH"
        # Seems we have frankingLicense invalid, don't really know how it works
        data['frankingLicense'] = "00000000"
    ...
    def generate_label(self, picking, packages, user_lang=None):
        ...
        if response.status_code != 200:
            res['success'] = False
            res['errors'] = ['Error when communicating with swisspost API']
            return res
