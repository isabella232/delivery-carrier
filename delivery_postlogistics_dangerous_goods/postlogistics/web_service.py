...
class PostlogisticsWebServiceDangerousGoods(
    web_service.PostlogisticsWebService):
    def _get_unnumbers(self, picking, pack=None):
        ...
        limited_amount_lq = picking.env.ref(
            "l10n_eu_product_adr.limited_amount_1")
        limited_quantity_products = products.filtered(
            lambda
                p: p.is_dangerous and p.limited_amount_id == limited_amount_lq
        )
        if limited_quantity_products:
            unnumbers = []
    ...
    def _prepare_attributes(self, picking, pack=None, pack_num=None,
